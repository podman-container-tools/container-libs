// Package hook is the 1.1.0 hook configuration structure.
package hook
...
func (hook *Hook) Validate(extensionStages []string) (err error) {
	...
	for key, value := range hook.When.AnnotationsOr {
		if _, err = regexp.Compile(key); err != nil {
			return fmt.Errorf("invalid annotation_or key %q: %w", key, err)
		}
		if _, err = regexp.Compile(value); err != nil {
			return fmt.Errorf("invalid annotation_or value %q: %w", value, err)
		}
	}
