...
func TestGoodRead(t *testing.T) {
	hook, err := Read([]byte("{\"version\": \"1.1.0\", \"hook\": {\"path\": \"/a/b/c\"}, \"when\": {\"always\": true}, \"stages\": [\"prestart\"]}"))
	if err != nil {
		t.Fatal(err)
	}
	always := true
...
func TestWrongVersion(t *testing.T) {
	...
	assert.Regexp(t, "^unexpected hook version \"0.1.0\" \\(expecting 1.1.0\\)$", err.Error())
}
