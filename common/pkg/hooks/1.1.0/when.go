...
type When struct {
	...
	AnnotationsOr map[string]string `json:"annotationsOr,omitempty"`
...
func (when *When) Match(config *rspec.Spec, annotations map[string]string, hasBindMounts bool) (match bool, err error) {
	...
	matched := false
	for keyPattern, valuePattern := range when.AnnotationsOr {
		for key, value := range annotations {
			keyMatch, err := regexp.MatchString(keyPattern, key)
			if err != nil {
				return false, fmt.Errorf("annotation_or key: %w", err)
			}
			valueMatch, err := regexp.MatchString(valuePattern, value)
			if err != nil {
				return false, fmt.Errorf("annotation_or value: %w", err)
			}
	...
	if len(when.AnnotationsOr) > 0 && !matched {
		return false, nil
	}
