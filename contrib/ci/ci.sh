...
limactl shell --workdir /var/tmp/container-libs $LIMA_VM_NAME ./contrib/ci/runner.sh "${@}"
rc=$?

limactl shell --workdir /var/tmp/container-libs $LIMA_VM_NAME sudo contrib/ci/logcollector.sh journal &> "$SCRIPT_DIR/journal.log"
